...
def compare_states(new_state, old_state, overlap_matrix):
    ...
    # Making use of Python's notion of truthiness
    states = range(len(overlap_matrix))
    excited_to = [i for i in states if new_state.Occupancy[i] and not old_state.Occupancy[i]]
    excited_from = [i for i in states if old_state.Occupancy[i] and not new_state.Occupancy[i]]
...
def make_p_vector(new_MOs, other_MOs, NElectrons, overlap_matrix):
    ...
    # MO_overlap = <ref|new>
    C_dagger = other_MOs[:,:NElectrons].T    # Is uing NElectrons giving the right number of rows?
